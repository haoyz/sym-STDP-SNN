...
{
  static int tmp_id = 0; //   1 -> 0 due to new Estimation in train.cc: revised at 2018-05-09 09:00  // revised at 2018-04-25 15:30
...
{
  static int tmp_id = 0; //   1 -> 0 due to new Estimation in train.cc: revised at 2018-05-09 09:00  // revised at 2018-04-25 15:30
  char plot_image[150];  //for no longer "line 0: undefined variable: _NExc_Sqrt"
  sprintf(plot_image, "plot \"./output/plot_visual_PEw_bin.dat\" binary array=(%d,%d) endian=little format=\"%%float\" with image",
          _NExc_Sqrt * _NPoi_Sqrt, _NExc_Sqrt * _NPoi_Sqrt); //否则会出现图片文件全部是0字节的错误
  ...
  if (_NExc <= 400)
    ...
    tmp_plotter("set terminal png size 640*5,480*5"); //wxt指屏幕，term是terminal缩写 // revised at 2018-04-26 21:50
  char tmp_commond[100];
  sprintf(tmp_commond, "set output \"./output/PEweight%d.png\"", tmp_id); //文本式3d图
  tmp_plotter(tmp_commond);
  // tmp_plot("replot"); //replot跑起来不能用
  tmp_plotter(plot_image); //for no longer "line 0: undefined variable: _NExc_Sqrt"
  tmp_id++;
}
void plot_visual_ECw_init(GNUplot &tmp_plotter)
{
  ...
  tmp_plotter("set size square"); //高宽比为1
  char tmp_cbrange[30];
  sprintf(tmp_cbrange, "set cbrange [0:%f]", (double)_g_EC_max);
...
void plot_variance_init(GNUplot &tmp_plot, int current_evaluation)
{
  ...
  tmp_plot("set xlabel \"number of examples()\"");
  ...
  if (_NExc <= 1600)
    ...
    tmp_plot("set yrange [0:300]"); // revised at 2018-04-26 21:50
  tmp_plot("set grid");
}
...
void plot_variance_E2C_init(GNUplot &tmp_plot, int current_evaluation)
{
  ...
  if (_NExc <= 1600)
    ...
    tmp_plot("set yrange [0:6000]"); // 600 -> 6000 revised at 2018-04-26 21:50
  else if (_NExc <= 10000)
    tmp_plot("set yrange [0:10000]"); // added - revised at 2018-04-26 21:50
  tmp_plot("set grid");
}
...
void plot_performance_init(GNUplot &tmp_pt, int num_examples) //做好画图的准备。画图的框框，准备performance存储实时的准确率。
{
  ...
  tmp_pt("set xlabel \"number of examples()\"");
  tmp_pt("set ylabel \"accuracy(%)\"");
  char tmp_commond[30];
  sprintf(tmp_commond, "set xrange [0:%d]", num_examples / 10000);
  tmp_pt(tmp_commond);
  tmp_pt("set yrange [0:100]");
  char tmp_commond_[30];
  sprintf(tmp_commond_, "set xtics 0,1,%d", num_examples / 10000);
  tmp_pt(tmp_commond_);
  tmp_pt("set ytics 0,5,100");
  tmp_pt("set grid");
}
void plot_performance(GNUplot &tmp_pt, int tmp) //代码重用//revised at 2018-4-18
{                                               //用上一个_performance_update_interval来计算此时准确率
  ...
  if (_NExc <= 400)
    ...
    tmp_pt("set terminal png size 640*5,480*5"); //wxt指屏幕，term是terminal缩写 // revised at 2018-04-26 21:50
...
{
  static int tmp_id_unsupervised = 0; //   1 -> 0 due to new Estimation in train.cc: revised at 2018-05-09 09:00  // revised at 2018-04-25 15:30
...
void plot_spikes_init(GNUplot &tmp_plot)
{
  ...
  char tmp_commond[30];
  sprintf(tmp_commond, "set xrange [-1:%d]", _RunTime_singleImg);
  tmp_plot(tmp_commond);
  char tmp_commond_[30];
  sprintf(tmp_commond_, "set yrange [-10:%d]", _NExc + 10);
  tmp_plot(tmp_commond_);
  char tmp_commond__[40];
  sprintf(tmp_commond__, "set xtics 0,%d,%d", 50, _RunTime_singleImg);
...
//   tmp_plotter("set size square"); //高宽比为1
//   char tmp_cbrange[30];
//   sprintf(tmp_cbrange, "set cbrange [0:%d]", (int)_g_EC_max);
//   tmp_plotter(tmp_cbrange); //色版对应取值范围
//   // tmp_plotter("set title \"visual ECw\"");
//   // tmp_plotter("set xlabel \"neuron ID\"");
//   // tmp_plotter("set ylabel \"weights\"");
...
//   tmp_pt("set grid");
// }
// void plot_performance(GNUplot &tmp_pt, int fileName) //代码重用//revised at 2018-4-18
// {                                                    //用上一个_performance_update_interval来计算此时准确率
//   char tmp_commond[150];
//   char file_name[50];
//   cout << fileName << endl;
...
//   tmp_plot("set xrange [-2:10]");
//   tmp_plot("set yrange [-2:10]");
//   tmp_plot("set xtics -1,1,9");
//   tmp_plot("set ytics -1,1,9");
//   tmp_plot("set terminal png");
// }
// void plot_confusion_m(GNUplot &tmp_plot)
// {
//   static int tmp_id = 0;
//   char tmp_commond[100];
//   cout << "plot_confusion_m" << endl;
//   sprintf(tmp_commond, "set output \"./output/confusion_m_%d.png\"", tmp_id);
//   tmp_plot(tmp_commond);
//   tmp_plot("set pm3d map");                                       //视角
//   tmp_plot("splot \"./output/plot_confusion_m.dat\" with image"); //文本式3d图
//   // tmp_plot("replot"); //replot跑起来不能用
//   tmp_id++;
// }
// void plot_spikes_init(GNUplot &tmp_plot)
